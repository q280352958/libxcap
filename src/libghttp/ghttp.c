...
char *
ghttp_digest_encode(ghttp_request *a_request,
                    const char *a_user,
                    const char *a_pass,
                    const char *digest_cache)
{
    char authEntity[1024];
    const char *AuthenticatePoint;
    char *RealmPoint, *RealmPointEnd, *Realm;
    char *NoncePoint, *NoncePointEnd, *Nonce;
    char *PoaquePoint, *PoaquePointEnd, *Poaque;
    char *QopPoint, *QopPointEnd, *Qop;
    char *AUTH = "auth";
    char MD5[] = "MD5";
    ...
    AuthenticatePoint = digest_cache;

    RealmPoint = strstr(AuthenticatePoint, "realm=\"") + 7;
    RealmPointEnd = strstr(RealmPoint, "\"");
    length = strlen(RealmPoint) - strlen(RealmPointEnd);
    Realm = (char *)malloc(length + 1);
    memset(Realm, 0, length + 1);
    memcpy(Realm, RealmPoint, length);

    NoncePoint = strstr(AuthenticatePoint, "digest nonce=\"") + 14;
    NoncePointEnd = strstr(NoncePoint, "\"");
    length = strlen(NoncePoint) - strlen(NoncePointEnd);
    Nonce = (char *)malloc(length + 1);
    memset(Nonce, 0, length + 1);
    memcpy(Nonce, NoncePoint, length);

    PoaquePoint = strstr(AuthenticatePoint, "opaque=\"") + 8;
    PoaquePointEnd = strstr(PoaquePoint, "\"");
    length = strlen(PoaquePoint) - strlen(PoaquePointEnd);
    Poaque = (char *)malloc(length + 1);
    memset(Poaque, 0, length + 1);
    memcpy(Poaque, PoaquePoint, length);

    QopPoint = strstr(AuthenticatePoint, "qop=\"") + 5;
    QopPointEnd = strstr(QopPoint, "\"");
    length = strlen(QopPoint) - strlen(QopPointEnd);
    Qop = (char *)malloc(length + 1);
    memset(Qop, 0, length + 1);
    memcpy(Qop, QopPoint, length);

    HTTPDigestGenerateCNonce(Cnonce);

    switch (a_request->req->type)
    {
    case http_req_type_put:
        Method = "PUT";
        break;
    case http_req_type_delete:
        Method = "DELETE";
        break;
    default:
        Method = "GET";
    ...
    memset(authEntity, 0, 1024);
    addElement(authEntity, "Digest username", a_user);
    addElement(authEntity, "realm", a_request->uri->host);
    addElement(authEntity, "nonce", Nonce);
    addElement(authEntity, "uri", a_request->uri->resource);
    addElement(authEntity, "response", Response);
    addElement(authEntity, "opaque", Poaque);
    addElement(authEntity, "algorithm", MD5);
    addElement(authEntity, "qop", AUTH);
    addElementNC(authEntity);
    addElement(authEntity, "cnonce", Cnonce);
